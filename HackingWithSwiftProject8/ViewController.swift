...
//
//  Created by user208023 on 10/24/22.
//
...
class ViewController: UIViewController {
    ...
    @objc func submitTapped(_ sender: UIButton) {
        ...
        if let solutionPosition = solutions.firstIndex(of: answerText) {
            ...
            guard let answerString = splitAnswers?.joined(separator: "\n") else { return }
            answersLabel.text = answerString
            rightAnswers.append(answerString)
            
            currentAnswer.text = ""
            score += 1
            
            if rightAnswers.count % 7 == 0 {
                let ac = UIAlertController(title: "Well Done!", message: "Are you ready for the next level?", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Let's go", style: .default, handler: levelUp))
                present(ac, animated: true)
            }
        } else {
            let ac = UIAlertController(title: "Wrong Answer!", message: "This is not the answer, try again!", preferredStyle: .alert)
